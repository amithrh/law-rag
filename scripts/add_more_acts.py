#!/usr/bin/env python3
"""Add 5 high-impact slice acts to fill known gaps (see chat 2026-05-17).

Acts being added:
  1. Bharatiya Nagarik Suraksha Sanhita 2023  (BNSS — criminal procedure, CrPC successor)
  2. Protection of Women from Domestic Violence Act 2005
  3. Bharatiya Sakshya Adhiniyam 2023          (Evidence Act successor)
  4. Indian Contract Act 1872
  5. Specific Relief Act 1963

Same pipeline as add_acts: IndiaCode search → handle page → PDF → PyMuPDF
→ PII redact → chunk → embed (sentence-transformers MPS) → postgres.
Idempotent on slug.
"""
...
from chunking.act import chunk_act, extract_as_at  # noqa: E402
from ingest.normalize.redact import redact  # noqa: E402

NEW_ACTS = [
    # (search query, slug)
    ("Bharatiya Nagarik Suraksha Sanhita 2023", "bnss-2023"),
    ("Protection of Women from Domestic Violence Act 2005", "domestic-violence-2005"),
    ("Bharatiya Sakshya Adhiniyam 2023", "sakshya-adhiniyam-2023"),
    ("Indian Contract Act 1872", "indian-contract-1872"),
...
def fetch_text(url: str) -> str:
    r = subprocess.run(["curl", "-sS", "-L", "--compressed", "--max-time", "25", *HEADERS, url],
                       capture_output=True, text=True, timeout=35)
    return r.stdout


def fetch_bytes(url: str, out_path: Path) -> int:
    r = subprocess.run(["curl", "-sS", "-L", "--compressed", "--max-time", "60",
                        "-w", "%{http_code}", *HEADERS, "-o", str(out_path), url],
                       capture_output=True, text=True, timeout=70)
    http_code = (r.stdout or "").strip()
    if not out_path.exists() or out_path.stat().st_size < 1000:
        # Either the file wasn't written, or it's a tiny error page
...
def resolve_handle(query: str) -> dict | None:
    ...
    for tr in soup.select("table.table tr, table.miscTable tr"):
        ...
        if a and len(cells) >= 3:
            ...
            candidates.append({"handle_id": hid, "collection": col,
                               "date": cells[0], "act_no": cells[1] if len(cells) > 1 else "",
                               "title": cells[2] if len(cells) > 2 else ""})
    year = query.split()[-1]
    matching_year = [c for c in candidates if year in c["date"] or year in c["title"]]
    central = [c for c in matching_year if c["collection"] == CENTRAL_COLLECTION]
    return central[0] if central else (matching_year[0] if matching_year else (candidates[0] if candidates else None))
...
async def main():
    ...
    # --- Step 1: resolve + download + extract -------------------------------
    ...
    for query, slug in NEW_ACTS:
        ...
        if not chosen:
            ...
            continue
        print(f"  ✓ {query} → handle={chosen['handle_id']} col={chosen['collection']} {chosen['title'][:50]}")
    ...
    for slug, query, chosen in resolved:
        handle_url = f"https://www.indiacode.nic.in/handle/123456789/{chosen['handle_id']}"
        ...
        print(f"     pages={n_pages}, chars={len(red.text):,}, pii_hits={red.suspicious}")
        time.sleep(1.0)
    ...
    print("  loading bge-m3 on MPS (may take time if first call)...")
    t0 = time.time()
    ...
    conn = await asyncpg.connect(host="localhost", port=int(env["POSTGRES_HOST_PORT"]),
    ...
    for slug, (meta, chunks) in by_slug.items():
        ...
        existing = await conn.fetchval("SELECT id FROM documents WHERE doc_id = $1", slug)
        if existing:
        ...
        src_id = await conn.fetchval(
            """INSERT INTO sources (source_type, origin, url, canonical_url_hash, metadata)
               ...
               ON CONFLICT (canonical_url_hash) DO UPDATE SET source_type = EXCLUDED.source_type
               RETURNING id""",
        ...
        # Insert chunks (dedupe within this act)
        seen = set()
        global_offset = 0
        for i, (other_slug, _) in enumerate(all_chunks):
            if other_slug["slug"] == slug:
                global_offset = i
                break
        local_idx = 0
        for c in chunks:
            if c.anchor in seen:
                continue
            seen.add(c.anchor)
            # Find this chunk's index in the full all_chunks list
            ...
            await conn.execute("""
                INSERT INTO chunks (document_id, source_type, anchor, token_count, text, embedding,
            ...
            """, doc_pk, c.anchor, c.token_count, c.text, emb_str,
                 c.chunk_strategy.value, c.as_at, json.dumps(c.metadata))
            inserted += 1
            local_idx += 1
        print(f"  ✓ {slug}: {len(seen)} chunks inserted")

    total_acts = await conn.fetchval("SELECT COUNT(*) FROM documents WHERE source_type IS NULL OR doc_id NOT LIKE '%insc%'")
    total_chunks = await conn.fetchval("SELECT COUNT(*) FROM chunks WHERE source_type='bare_act'")
    print(f"\n=== Summary ===")
    print(f"  inserted this run: {inserted} chunks, skipped (dup): {skipped}")
    print(f"  DB totals: {total_chunks} bare_act chunks across all acts")
    await conn.close()
